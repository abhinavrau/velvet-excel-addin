...
export async function createSummarizationData(config) {
  ...
  await Excel.run(async (context) => {
    try {
      ...
      const summarization_verbosityColumn = getColumn(testCasesTable, "summarization_verbosity");
      ...
      while (
        ...
        appendLog(`sumCaseID:: ${id[processedCount][0]} Started Processing...`);
        ...
        promiseMap.set(
          processedCount,
          callGeminiMultitModal(processedCount, full_prompt, null, null, config)
            .then(async (result) => {
...
async function processResponse(
  ...
  appendLog(`sumCaseID::${rowNum} summarizationQualityResult Started..`);
  // summary quality
  ...
  appendLog(`sumCaseID::${rowNum} summarizationHelpfulness Started..`);
  // summary helpfulness
...
async function callSummaryEval(
  eval_url,
  token,
