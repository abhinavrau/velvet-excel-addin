    ...
    await Excel.run(async (context) => {
        try {
            ...
            if (fileUriColumn.isNullObject || idColumn.isNullObject) {
                showStatus(`Error in createSyntheticQAData: No fileUriColumn or ID column found in Test Cases Table. Make sure there is an ID and Query column in the Test Cases Table.`, true);
                return;
            }
            ...
            while (processedCount <= countRows && id[processedCount][0] !== null && id[processedCount][0] !== "") {
                ...
                appendLog(`genSynQID: ${id[processedCount][0]} Start Processing.`);
                ...
                promiseMap.set(processedCount, callGeminiMultitModal(processedCount, prompt, fileUri[processedCount][0], mimeType[processedCount][0], config)
                    .then(async result => {
                        ...
                        if (status == 200) {
                            await processResponse(rowNum, output, generatedQuestionColumn, expectedAnswerColumn, reasoningAColumn , statusColumn, responseTimeColumn,  config, context);
...
async function processResponse(rowNum, output, generatedQuestionColumn, expectedAnswerColumn, reasoningColumn, statusColumn, responseTimeColumn, config, context) {
